...
struct heston_parms {
	double K; 
	double x;      // initial lnS
	double r; 
	double v;      // initial volatility
	double tau; 
	double kappa; 
	double theta;  // mean variance
	double rho; 
	double sigma;  // vol of vol
	double lambda; // sharpe ratio
	int j;
};
...
double heston_integrand_j(double phi, void *p) {
	...
	double theta = (parms->theta); double rho = (parms->rho);
	double sigma = (parms->sigma); double lambda = (parms->lambda); double tau = (parms->tau);
	double j = (parms->j);
...
inline double heston_Pj(double S, double K, double r, double v, double tau,
	 double sigma, double kappa, double lambda, double rho, double theta, int j) {
...
double heston_call_option_price(const double& S, const double& K, 
	const double& r, const double& v, const double& tau,
	const double& rho, const double& kappa, const double& lambda, 
